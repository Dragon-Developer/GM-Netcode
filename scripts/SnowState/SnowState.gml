...
function SnowState(_initState, _execEnter = true) constructor {
	...
	__add = function(_name, _struct, _hasParent) {
		...
		if (_hasParent) {
			...
			_events = struct_get_names(_struct);
	...
	__create_events_struct = function(_struct) {
		...
		_arr = struct_get_names(__defaultEvents);
		_i = 0; repeat(array_length(_arr)) {
			_event = _arr[_i];
	...
	__update_states = function(_hasParent) {
		...
		_states = struct_get_names(__states);
		_events = struct_get_names(__defaultEvents);
	...
	get_states = function() {
		return struct_get_names(__states);	
	};
...
#macro struct_get_names struct_get_names_fixed
function struct_get_names_fixed(_struct) {
	var _arr = variable_struct_get_names(_struct);
	var _result = [];
	var _i = 0; repeat(array_length(_arr)) {
		if (!string_starts_with(_arr[_i], "___struct___")) {
			array_push(_result, _arr[_i]);	
		}
		_i++;
	}
	return _result;
